...
class FlaskTestCase(unittest.TestCase):
	...
	def test_login_page_loads(self):
		...
		self.assertTrue(b'Log In to Murasaki Bank' in response.data)

	# Check signup page loads correctly
	def test_signup_page_loads(self):
		tester = self.app
		response = tester.get('/signup', content_type='html/text')
		self.assertTrue(b'Sign Up For Murasaki Bank' in response.data)
	...
	def test_admin_panel(self):
		...
		response = tester.get('/murasaki/admin', content_type='html/text')
	...
	def test_internal_money_transfer_send(self):
		...
		response = tester.post('/murasaki', data=dict(email=self.test_user_2.email, amount=200), follow_redirects=True)
	...
	def test_internal_money_transfer_wrong_email(self):
		tester = self.app
		tester.post('/login', data=dict(email=self.test_user_1.email, password='password'), follow_redirects=True)
		response = tester.post('/murasaki', data=dict(email='wrong@email.com', amount=200), follow_redirects=True)
	...
	def test_internal_money_transfer_too_many_decimals(self):
		tester = self.app
		tester.post('/login', data=dict(email=self.test_user_1.email, password='password'), follow_redirects=True)
		response = tester.post('/murasaki', data=dict(email=self.test_user_2.email, amount=200.111), follow_redirects=True)
	...
	def test_internal_money_transfer_too_much_money(self):
		tester = self.app
		tester.post('/login', data=dict(email=self.test_user_1.email, password='password'), follow_redirects=True)
		response = tester.post('/murasaki', data=dict(email=self.test_user_2.email, amount=20000),
	...
	def test_internal_money_transfer_transaction_log(self):
		...
		response = tester.post('/murasaki', data=dict(email=self.test_user_2.email, amount=20),
							   follow_redirects=True)
